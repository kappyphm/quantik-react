...
export default function Home({go,apiReady=false}){
  ...
  return <div className="page home">
    <div className="home-copy"><div className="eyebrow"><span className="eyebrow-line"/> QUANTIK / VIETNAM EQUITIES</div>
      ...
      <div className="home-meta"><span>ĐỢT CÔNG BỐ GẦN NHẤT</span><strong>{apiReady?latest?.published_at?.slice(0,16).replace('T',' ')||'Chưa có':run.published}</strong><span className="meta-sep"/><span>{apiReady?latest?.universe_count??'—':stocks.length} MÃ {apiReady?'TRONG BẢN QUÉT':'TRONG BẢN DEMO'}</span>{apiReady&&scanStatus?.status==='running'&&<span> · ĐANG QUÉT THẬT {scanStatus.progress_pct}%</span>}{apiReady&&scanStatus?.status==='failed'&&<span> · QUÉT LỖI</span>}</div>
    </div>
