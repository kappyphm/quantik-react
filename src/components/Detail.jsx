...
export default function Detail({ symbol, go, startJob, jobs }) {
  ...
  return <div className="page detail-page">
    ...
    <div className="detail-head"><div><div className="eyebrow">{s.exchange} / {s.sector} / {s.run_id}</div><h1><span>{s.symbol}</span> <small>{s.name}</small></h1><p>Kết quả đánh giá từ đợt đã công bố · dữ liệu đến {s.data_as_of}</p></div><button className="primary" onClick={() => startJob(symbol)}>CHẠY PHÂN TÍCH QUANT <span>↗</span></button></div>
    ...
    <div className="detail-grid"><section className="analysis-panel"><div className="section-title"><span>01 / BIỂU ĐỒ GIÁ & KHỐI LƯỢNG</span><small>Lightweight Charts · snapshot đã công bố</small></div><div className="stock-chart">{ohlcv?.bars?.length ? <PriceChart symbol={symbol} ohlcvBars={ohlcv.bars} /> : <div className="market-api-empty">Chưa có OHLCV cho mã này.</div>}</div></section><div className="detail-side"><section className="analysis-panel"><div className="section-title"><span>02 / LUẬN ĐIỂM</span></div><p className="reason">{s.gateExplanation || s.commentary || 'Chưa có luận điểm chi tiết.'}</p><div className="text-grid"><span>Đánh giá</span><strong>{s.rating}</strong><span>Xu hướng VN-Index</span><strong className={tone(s.vniTrend)}>{s.vniTrend}</strong><span>Xu hướng ngành</span><strong className={tone(s.sectorTrend)}>{s.sectorTrend}</strong><span>Nhóm ngành</span><strong>{s.sector}</strong></div></section><section className="analysis-panel"><div className="section-title"><span>03 / ĐIỀU KIỆN SÀNG LỌC</span><small>Snapshot đã công bố</small></div><div className="check-list">{!checks.length && <div className="empty-history">Chưa có chi tiết điều kiện sàng lọc.</div>}{checks.map(([label, passed]) => <div key={label}><span>{label}</span><strong className={passed ? 'positive' : 'negative'}>{passed ? '✓ Đạt ngưỡng' : '× Chưa đạt'}</strong></div>)}</div></section><section className="analysis-panel quant-cta"><div className="eyebrow">PHÂN TÍCH CHUYÊN SÂU</div><h2>Cần nhiều hơn một tín hiệu?</h2><p>Chạy mô hình QUANT và tạo biểu đồ báo cáo cho {symbol}.</p><button className="primary" onClick={() => startJob(symbol)}>CHẠY PHÂN TÍCH QUANT ↗</button></section></div></div>
