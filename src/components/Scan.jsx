...
export default function Scan({go,apiReady=false}){
  ...
  useEffect(()=>{if(!apiReady)return;let active=true;const refresh=()=>{fetch('/api/v1/scans/latest').then(r=>r.ok?r.json():null).then(data=>{if(active)setActiveRun(data);}).catch(()=>{});fetch('/api/v1/scans/status').then(r=>r.json()).then(data=>{if(active)setScanStatus(data);}).catch(()=>{});};refresh();const timer=setInterval(refresh,15000);return()=>{active=false;clearInterval(timer);};},[apiReady]);
  useEffect(()=>{if(!apiReady||!activeRun)return;fetch('/api/v1/scans/latest/facets').then(r=>r.ok?r.json():null).then(setFacets).catch(()=>{});},[apiReady,activeRun?.id]);
  useEffect(()=>{
    if(!apiReady)return;
  ...
  },[apiReady,activeRun?.id,query,rec,gate,exchange,sector,min,sort,asc,page,pageSize]);
  ...
  return <div className="page scan-page"><div className="page-heading"><div><div className="eyebrow">PHÂN TÍCH TOÀN SÀN / BẢN CÔNG BỐ MỚI NHẤT</div><h1>Kết quả quét <span>toàn sàn</span></h1><p>{apiReady&&!activeRun?'Chưa có bản quét thật được công bố. Tiến độ đợt đang chạy hiển thị bên dưới.':'Dữ liệu từ đợt phân tích đã công bố. Chọn một mã để xem chi tiết đánh giá.'}</p></div><div className="run-stamp"><span>RUN ID</span><strong>{apiReady?activeRun?.id||scanStatus?.run_id||'—':run.id}</strong><small>{apiReady?activeRun?.published_at?.slice(0,16).replace('T',' ')||'Chưa công bố':run.published}</small></div></div>
    <div className="scan-stats"><div><span>TRẠNG THÁI</span><strong className={activeRun||!apiReady?'positive':''}>{apiReady?activeRun?'● Đã công bố':scanStatus?.status==='running'?`Đang quét ${scanStatus.progress_pct}%`:scanStatus?.status==='failed'?'Quét thất bại':'Chưa có bản quét thật':'● Đã công bố'}</strong></div><div><span>PHIÊN PHÂN TÍCH</span><strong>{apiReady?activeRun?.slot||scanStatus?.slot||'—':run.slot}</strong></div><div><span>DỮ LIỆU ĐẾN</span><strong>{apiReady?activeRun?.data_as_of||'—':run.asOf}</strong></div><div><span>SỐ MÃ HIỂN THỊ</span><strong>{total} <i>/ {apiReady?activeRun?.universe_count??0:stocks.length}</i></strong></div></div>
