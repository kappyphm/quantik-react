...
const stamp = value => value ? new Date(value).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }) : 'Chưa công bố';
...
export default function Scan({ go }) {
  ...
  return <div className="page scan-page">
    <div className="page-heading"><div><div className="eyebrow">PHÂN TÍCH TOÀN SÀN / BẢN CÔNG BỐ MỚI NHẤT</div><h1>Kết quả quét <span>toàn sàn</span></h1><p>{activeRun ? 'Dữ liệu từ đợt phân tích đã công bố. Chọn một mã để xem chi tiết đánh giá.' : 'Chưa có bản quét thật được công bố. Tiến độ đợt đang chạy hiển thị bên dưới.'}</p></div><div className="run-stamp"><span>RUN ID</span><strong>{activeRun?.id || scanStatus?.run_id || '—'}</strong><small>{stamp(activeRun?.published_at)}</small></div></div>
