...
class ScreenerBridge:
    ...
    def fetch_index(self, idx='VNINDEX', days=252):
        ...
        for src in self.chain:
            try:
                try:
                    raw = self.market.index(symbol=idx).ohlcv(start=start, end=end, interval='1D', source=src, count=500)
                except Exception:
                    # Compatibility fallback for older vnstock_data versions.
                    raw = self.market.equity(symbol=idx).ohlcv(start=start, end=end, interval='1D', source=src, count=500)
